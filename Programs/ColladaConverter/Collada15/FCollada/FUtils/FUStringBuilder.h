...
public:
    ...
    template <class T>
    inline void appendHex(const T& i)
    {
        for (size_t j = 0; j < sizeof(T); ++j) appendHex(*(((uint8*)&i) + j));
    } /**< See above. */
...
#ifdef _W64
    inline void append(_W64 unsigned int i)
    {
        append((uint32)i);
    } /**< See above. */
#else
...
#endif
#else
    inline void append(unsigned long i)
    {
        append((uint32)i);
    } /**< See above. */
    inline void append(long i)
    {
        append((int32)i);
    } /**< See above. */
    ...
    inline FUStringBuilderT& operator+=(const TYPE& val)
    {
        append(val);
        return *this;
    }
