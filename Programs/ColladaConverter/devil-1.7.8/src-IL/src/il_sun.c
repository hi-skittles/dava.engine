...
ILboolean ilIsValidSun(ILconst_string FileName)
{
    ...
    if (!iCheckExtension(FileName, IL_TEXT("sun")) && !iCheckExtension(FileName, IL_TEXT("ras")) &&
        ...
        !iCheckExtension(FileName, IL_TEXT("rs")))
    { // Lots of names possible...
...
ILboolean iLoadSunInternal(void)
{
    ...
    case 8:
        if (Header.ColorMapType == IL_SUN_NO_MAP)
        { // Greyscale image
        ...
        { // Colour-mapped image
            ...
            if (iCurImage->Pal.Palette == NULL)
                return IL_FALSE;
            if (iread(iCurImage->Pal.Palette, 1, Header.ColorMapLength) != Header.ColorMapLength)
            { // Read color map.
        ...
        if (Header.Type != IL_SUN_BYTE_ENC)
        { // Regular uncompressed image data
...
ILuint iSunGetRle(ILubyte* Data, ILuint Length)
{
    ...
    for (i = 0; i < Length;)
    {
        Flag = igetc();
        if (Flag == 0x80)
        { // Run follows (or 1 byte of 0x80)
            Count = igetc();
            if (Count == 0)
            { // 1 pixel of value (0x80)
                *Data = 0x80;
                Data++;
                i++;
            }
        ...
        { // 1 byte of this value (cannot be 0x80)
            *Data = Flag;
            Data++;
            i++;
        }
