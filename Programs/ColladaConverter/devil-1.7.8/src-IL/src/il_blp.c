...
ILboolean iLoadBlpInternal(void)
{
    ...
    case BLP_RAW:
        ...
        { // Possible maximum of 16 mipmaps
            if (BaseCreated)
            {
            ...
            case 0:
                if (!BaseCreated)
                { // Have not created the base image yet, so use ilTexImage.
            ...
            case 1:
                ...
                for (y = Image->Height - 1; y >= 0; y--)
                {
                    ...
                    {
                        if (AlphaMask == 0)
                        { // Shifting it past the highest bit makes it 0, since we only have 1 byte.
